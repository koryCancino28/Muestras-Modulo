...
<body>
    ...
    <script>
        // Mostrar mensaje en móviles (touchstart para detectar toques en la pantalla)
        $('.aprobado-coordinadora, .aprobacion-jefe').on('click touchstart', function(e) {
                if ($(this).prop('disabled')) {
                    e.preventDefault(); // Evita que se active el checkbox
        ...
        $(document).ready(function() {
            // Habilitar el checkbox de la coordinadora solo si el jefe comercial ha aprobado
            $('.aprobacion-jefe').on('change', function() {
                var isChecked = $(this).is(':checked');
                var id = $(this).closest('tr').find('.aprobado-coordinadora').data('id');

                // Habilitar o deshabilitar el checkbox de coordinadora
                $('.aprobado-coordinadora[data-id="' + id + '"]').prop('disabled', !isChecked);
            });

            // Mostrar mensaje en computadoras (hover)
            ...
            // Evento para manejar cambios en el checkbox de la coordinadora
            $('.aprobado-coordinadora').on('change', function() {
                var id = $(this).data('id');
                var value = $(this).is(':checked') ? 1 : 0;
                actualizarAprobacion(id, 'aprobado_coordinadora', value);
            });

            // Función AJAX para actualizar la aprobación
...
channel.bind('muestra.creada', function(data) {
    ...
    setTimeout(function() {
        window.location.href = '/muestras/Coordinadora';
    }, 1000);
});
